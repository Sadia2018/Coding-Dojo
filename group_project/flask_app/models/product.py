...
class Product:
    ...
    def make_product(cls, form_data):
        query = "INSERT INTO products(name, item, description, users_id) VALUES (%(name)s, %(item)s, %(description)s,%(users_id)s)"
    ...
    def update_product (cls, data):
        query = "UPDATE products SET name=%(name)s, item=%(item)s, description=%(description)s  WHERE users_id = %(users_id)s"
